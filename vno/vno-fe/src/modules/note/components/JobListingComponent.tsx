...
export default function JobListingComponent({ notes }: { notes: NoteOverview[] }) {
  const [activeItem, setActiveItem] = useState<NoteOverview | null>(null);
  const [ct, setCt] = useState('<p>Hello World!</p>');

  const ref = useRef<HTMLDivElement>(null) as React.RefObject<HTMLDivElement>;
  useOnClickOutside(ref, () => setActiveItem(null));

  const handleItemClick = (note: NoteOverview) => {
    setActiveItem(note); // hoặc bất kỳ xử lý nào bạn muốn
  };
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {activeItem ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={` dark:bg-dark-50/10 pointer-events-none absolute inset-0 z-10 bg-blend-luminosity backdrop-blur-sm`}
          />
        ) : null}
      </AnimatePresence>
      {/* render active item */}
        ...
        {activeItem ? (
          <>
            <div className="group absolute inset-0 z-10 grid place-items-center ">
              <motion.div
                className={` ${activeItem.color}  dark:bg-dark-50 h-11/12 overflow-hidden p-4 shadow-xs`}
                ref={ref}
                layoutId={`workItem-${activeItem.title}`}
                style={{ borderRadius: 12, backgroundColor: `${activeItem.color}` }}
              >
                {typeof activeItem.color}
                {/* <div className="flex w-full items-center gap-4">
                  <motion.div layoutId={`workItemLogo-${activeItem.company}`}>{activeItem.logo} </motion.div>
                ...
                >
                  <QuillEditor value={ct} onChange={setCt} />
                  || <p className={`${activeItem.color}`}>{activeItem.color}</p>
                </motion.div>
              </motion.div>
            </div>
          </>
      ...
      <div className="relative flex items-start p-6">
        <div className="relative grid grid-cols-4 gap-8 px-2">
          {notes.map((note) => (
            // <motion.div
            ...
            //       {role.remote === 'Hybrid' && ` ${role.remote} / ${role.location} `}| {role.job_time}
            //     </motion.div>
            //   </div>
            // </motion.div>
            <SmoothItem key={note.id} note={note} onClick={handleItemClick} />
          ))}
        </div>
      </div>
    </>
