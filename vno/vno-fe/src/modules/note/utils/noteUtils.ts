//Nếu dùng Redux Toolkit hoặc slice state management cho note
...
export const getTimeAgo = (dateInput: Date | string): string => {
  ...
  // console.log('diffTime', Math.floor(diffTime / (1000 * 3600)));
...
export const groupNotesByDate = (notes: NoteOverview[]): ListNoteGroupedByDate[] => {
  const groupedObj = notes.reduce<{ [key: string]: NoteOverview[] }>((groups, note) => {
  ...
  const groupedArray: ListNoteGroupedByDate[] = Object.entries(groupedObj).map(([dateStr, notes]) => {
